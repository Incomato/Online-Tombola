...
interface EditPrizeModalProps {
  prize: Prize;
  onUpdate: (updatedPrize: Prize) => void;
  onClose: () => void;
}

const EditPrizeModal: React.FC<EditPrizeModalProps> = ({ prize, onUpdate, onClose }) => {
  ...
  const validate = (): boolean => {
    const newErrors = { name: '', description: '', ticketPrice: '' };
    let isValid = true;
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-3xl font-bold text-white mb-6">Edit Prize</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
           ...
           <div>
            ...
            {formData.image && <img src={formData.image} alt="Prize Preview" className="mt-2 rounded-md max-h-40 w-full object-cover" />}
            <input
              type="file"
              id="image"
              name="image"
